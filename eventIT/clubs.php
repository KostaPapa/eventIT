...
<body>
	...
	<?php
		if(isset($_SESSION['name'])) {
			echo '<table class="table"><tr><th></th><th>Club Name</th><th>Contact Email</th><th>Description</th>';
			if ($stmt = $mysqli -> query("SELECT cname, contact_email, description FROM club")) {
				if ($stmt->num_rows > 0)
					while ($row = $stmt->fetch_assoc()){
						...
						echo "<td>" . $row["cname"] . "</td><td>" . $row["contact_email"] . "</td><td>" . $row["description"] . "</td></tr>";
					}
			}
			echo '</table>';
		} else {
